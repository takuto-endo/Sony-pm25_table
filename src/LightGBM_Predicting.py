...
def predicting(same_reference_path=None, load_model=False):
    ...
    # model load lightgbm
    fnames = glob.glob(save_path+"/lgbm_model/*.txt")
    inference_times = []
    model_sizes = []
    for i, fname in enumerate(fnames):
        ...
        model_size = sys.getsizeof(bst)
        model_sizes.append(model_size)
        print(f"Model size: {model_size} byte.")
        preds.append(pred)
    print(f"\n[CHECK POINT]: END \"whole inference of lightgbm.\" mean_of_inference_time={np.mean(inference_times)}\n")
    ...
    submit_df[1] = preds
    submit_df.to_csv(save_path+"/lgbm_submit_file_"+reference_path+".csv",index=False, header=False)
    print("[CHECK POINT]: END save submission file >> [", save_path+"/lgbm_submit_file_"+reference_path+".csv ]")

    return save_path+"/lgbm_submit_file_"+reference_path+".csv"
