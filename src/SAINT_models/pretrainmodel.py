...
class SAINT(nn.Module):
    def __init__(
        self,
        *,
        categories,# 各変数のembedding size ex) [1, 46, 7, 12]
        num_continuous,# # 連続値変数の数
        dim,# 1変数のembedding size
        depth,# L: Transformer Blockの積み重ねる数
        heads,# multi headの数 > Attentionで使用
        dim_head = 16,# 1head毎の次元数 > Attentionの中間次元はheads*dim_head
        dim_out = 1,
        ...
        l = input_size // 8
        hidden_dimensions = list(map(lambda t: l * t, mlp_hidden_mults))# default: [4,2]*(embedding後のinput_size//8)
        all_dimensions = [input_size, *hidden_dimensions, dim_out]
        
        self.mlp = MLP(all_dimensions, act = mlp_act)# 使われてなくね??
        ...
        self.mlpfory = simple_MLP([dim ,1000, y_dim])# transformerの出力から(dim次元のCLS token情報)からy_dimのクラスor回帰を予測
